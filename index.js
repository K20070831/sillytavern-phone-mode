(async function () {
    ...
    async function fetchSMS(userMsg) {
        const c = getCtx();
        if (!c || !c.generateQuietPrompt) return ["（系统模块未准备好，请检查酒馆状态）"];

        conversationHistory.push({ role: 'user', content: userMsg });

        const systemPrompt = `【最高系统指令：强制切入短信模式】
你必须完全抛弃现有的ECoT、写作指导和小说体格式。
当前身份："${currentPersona}"。
要求：
...
5. 允许使用 (图片+内容) 或 (转账+数字)。`;
    ...
    // ── 8. 样式 ──
    const css = 
        '#pm-iphone-v12 { position: fixed; bottom: 40px; right: 40px; width: 330px; height: 580px; background: #fff; border: 10px solid #111; border-radius: 45px; z-index: 100000; display: flex; flex-direction: column; overflow: hidden; box-shadow: 0 20px 50px rgba(0,0,0,0.4); transition: 0.35s cubic-bezier(0.18, 0.89, 0.32, 1.2); font-family: -apple-system, BlinkMacSystemFont, sans-serif; touch-action: none; } ' +
        '#pm-iphone-v12.is-min { height: 48px; width: 130px; border-radius: 24px; border-width: 6px; } ' +
        '#pm-iphone-v12.is-min .pm-main-ui { display: none; } ' +
    ...
    if (!document.getElementById('pm-v12-css')) {
        const s = document.createElement('style'); s.id = 'pm-v12-css'; s.innerHTML = css; document.head.appendChild(s);
    }
    ...
    console.log("iPhone SMS V12 (Ultra-Compatible & Safe) Loaded.");
})();
